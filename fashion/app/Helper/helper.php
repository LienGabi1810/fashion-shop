...
function showCategory($mang,$parent,$shift){
	foreach($mang as $row){
		if($row['parent_id']==$parent){
			echo '<div class="item-menu fl"  style="display:flex;justify-content:space-between">
                    ...
                    <a onclick="return del()" class="btn-category btn-danger" href="/admin/category/category-delete/'.$row['id'].'"><i class="fas fa-times"></i></i></a>
                    </div>
                </div>';
